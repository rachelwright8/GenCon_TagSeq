...
assay(vsd) <- removeBatchEffect(assay(vsd), vsd$treat)
...
module_color <- "green" # replace with each module color and run the lines below

color %>% select(gene, moduleColor,paste("MM.",module_color,sep="")) %>%
  mutate(kME = ifelse(moduleColor==module_color, MM.green, 0)) %>% # you have to manually change the color here
  select(gene, kME) %>%
